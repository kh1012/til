...
- fix(assistant): screenContext 전환 후 남은 toAgentContext 제거
...
- refactor(assistant): 컨텍스트 store를 subscribed/snapshot으로 정리
...
- refactor(assistant): 생산 훅을 Url/Screen/Field로 명명
...
- refactor(assistant): 소비 훅을 Subscribed/Snapshot으로 명명
...
- refactor(assistant): 컨텍스트 조각 타입을 Fragment로 명명
...
- refactor(assistant): 화면 컨텍스트 훅을 ViewContext로 명명
...
- feat(assistant): 입력 맥락 태그를 포커스 필드·화면 2단으로 확장
...
- refactor(assistant): 입력 맥락 태그를 breadcrumb 구조로 전환
