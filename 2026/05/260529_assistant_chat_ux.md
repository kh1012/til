...
- fix(markdown): 한글 볼드 미렌더 CJK 버그 수정
...
- style(markdown): 코드 블록을 단색 단일 박스로 평탄화
...
- fix(tooltip): 툴팁이 패널 위에 뜨도록 z-index를 Positioner로 이동
...
- feat(assistant): 응답 진행 상태 atom 다리와 RNB 진입 스피너 추가
...
- feat(assistant): thread 목록 개편과 새 대화 컴포저 포커스 추가
...
- fix(assistant): 스트리밍 중 thread 전환 잠금으로 응답 덮어쓰기 방지
...
- fix(tooltip): delay를 Provider로 이동해 지연 시간 적용 정상화
...
- style(assistant): 메시지 타임스탬프 글자 크기 한 단계 확대
