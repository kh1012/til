...
- [ ] graphify CLI 설치 후 `src/widgets/` 한정으로 코드 그래프만 빌드해 보기
- [ ] 빌드 결과에서 FSD import 방향 위반이 잡히는지 검증
- [ ] `DESIGN.md` ingest 후 "디자인 토큰을 사용하는 컴포넌트" 쿼리 시도
- [ ] 토큰 절감 효과를 `/max-pm` 한 번 실행 기준으로 정량 측정

### 참고 자료

- 본 TIL은 Claude로 사전 조사한 내용을 기반으로 정리. 1차 출처는 graphify 공식 문서·런칭 블로그(2026-04-03 공개)와 사용자 사례 글들.
- 실제 사용 전 공식 저장소·문서에서 v0.4.2 기준 API와 출력 포맷을 직접 확인할 것 (버전 간 변경 가능성 있음).
